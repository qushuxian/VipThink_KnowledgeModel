...
def random_model_uni(num_resources=None,
    ...
    if num_resources is None:
        num_resources = 1
        print('num_resources未接收到传入参数，采用默认参数num_resources=%s' % num_resources)
    print('num_resources接收到传入参数num_resources=%s' % num_resources)

    if num_subparts is None:
        num_subparts = 1
        print('num_subparts未接收到传入参数，采用默认参数num_subparts=%s' % num_subparts)
    print('num_subparts接收到传入参数num_subparts=%s' % num_subparts)
    ...
    if trans_prior is None:
        ...
        print('trans_prior未接收到传入参数矩阵，采用默认参数矩阵trans_prior=\n%s' % trans_prior)
    else:
        trans_prior = trans_prior
        print('trans_prior接收到传入参数矩阵trans_prior=\n%s' % trans_prior)

    if given_notknow_prior is None:
        given_notknow_prior = np.tile([[5], [0.5]], (1, num_subparts))
        print('given_notknow_prior未接收到传入参数矩阵，采用默认参数矩阵given_notknow_prior=\n%s' % given_notknow_prior)
    else:
        given_notknow_prior = given_notknow_prior
        print('given_notknow_prior接收到传入参数矩阵given_notknow_prior=\n%s' % given_notknow_prior)

    if given_know_prior is None:
        given_know_prior = np.tile([[0.5], [5]], (1, num_subparts))
        print('given_know_prior未接收到传入参数矩阵，采用默认参数矩阵given_know_prior=\n%s' % given_know_prior)
    else:
        given_know_prior = given_know_prior
        print('given_know_prior接收到传入参数矩阵given_know_prior=\n%s' % given_know_prior)

    if pi_0_prior is None:
        pi_0_prior = np.array([[100], [1]])
        print('pi_0_prior未接收到传入参数矩阵，采用默认参数矩阵pi_0_prior=\n%s' % pi_0_prior)
    else:
        pi_0_prior = pi_0_prior
        print('pi_0_prior接收到传入参数矩阵pi_0_prior=\n%s' % pi_0_prior)
